...
@Named
@ViewScoped
public class HospitalEngineerDashboardBean extends MasterDashboardBean {
  @Inject
  ...
  public int totalRepairTasksByUser(int userId) {
    if (userId == 0) {
      ...
      if (this.totalRepairMyTask == 0) {
        this.totalRepairMyTask = this.masterDashboardService.getTotalRepairCurrentMonthOfMyTaskByHospPsnCode(userId);
      }
  ...
  public int totalRepairDoingTasksByUser(int userId) {
    if (userId == 0) {
      ...
      if (this.totalRepairDoingMyTask == 0) {
        this.totalRepairDoingMyTask = this.masterDashboardService.getTotalRepairDoingMyTaskByHospPnsCode(userId);
      }
  ...
  public double totalRepairCostTasksByUser(int userId) {
    if (userId == 0) {
      ...
      if (this.totalRepairCostMyTask == 0) {
        this.totalRepairCostMyTask = this.masterDashboardService.getTotalRepairCostCurrentMonthOfMyTaskByHospPnsCode(userId);
      }
  ...
  public List<ActionLog> listActionLogOfTasksByUser(int userId) {
    if (userId == 0) {
      ...
      if (this.listActionLogOfMyTask == null) {
        this.listActionLogOfMyTask = this.masterDashboardService.getListContactOfMyTaskByHospPnsCode(this.userCredential.getUserId());
      }
